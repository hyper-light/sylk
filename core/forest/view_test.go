...
func TestMemoryForestSnapshotGroupsTreesAndSelection(t *testing.T) {
	...
	if snapshot.Trees[0].Label != "Decision Tree" {
		t.Fatalf("tree[0].label = %q, want %q", snapshot.Trees[0].Label, "Decision Tree")
	}
	...
	if got := len(snapshot.Trees[0].Nodes); got != 2 {
		t.Fatalf("decision node count = %d, want 2", got)
	}
	if snapshot.Trees[0].Nodes[0].ID != "decision-1" || snapshot.Trees[0].Nodes[1].ID != "decision-2" {
		t.Fatalf("decision nodes order = [%s %s], want [decision-1 decision-2]", snapshot.Trees[0].Nodes[0].ID, snapshot.Trees[0].Nodes[1].ID)
	}
	if !snapshot.Trees[0].Nodes[1].Leaf {
		t.Fatal("expected newest decision node to be a leaf")
	}
