...
func TestSessionWorkspaceViews_GlobalReviewSourceReadsReviewOverlay(t *testing.T) {
	dir := t.TempDir()
	target := filepath.Join(dir, "hello.txt")
	...
	ctx := WithSessionID(context.Background(), "sess-1")
	if err := svfs.GlobalVFS().Write(ctx, target, []byte("checkpoint")); err != nil {
		t.Fatalf("seed global: %v", err)
	}
	pipe, err := svfs.BeginPipeline(BeginPipelineConfig{
		PipelineID: "task-1",
	...
	if err := pipe.Write(ctx, target, []byte("review")); err != nil {
	...
	if candidate == nil {
		t.Fatal("expected review candidate")
	}
	if err := svfs.ActivateReviewCandidate(ctx, candidate.ID); err != nil {
		t.Fatalf("ActivateReviewCandidate: %v", err)
	}

	checkpointViews := NewSessionWorkspaceViews(SessionWorkspaceViewsConfig{
		DefaultView:  WorkspaceViewGlobal,
	...
	reviewViews := NewSessionWorkspaceViews(SessionWorkspaceViewsConfig{
		DefaultView:  WorkspaceViewGlobal,
		GlobalSource: WorkspaceGlobalSourceReview,
		Session:      svfs,
		WorkingDir:   dir,
		DiskFallback: NewDiskFileAccess(dir, true),
	})

	checkpointContent, err := checkpointViews.ReadFile(ctx, WorkspaceViewGlobal, "hello.txt", "")
	if err != nil {
		t.Fatalf("checkpoint read: %v", err)
	}
	reviewContent, err := reviewViews.ReadFile(ctx, WorkspaceViewGlobal, "hello.txt", "")
	if err != nil {
		t.Fatalf("review read: %v", err)
	}
	if got := string(checkpointContent); got != "checkpoint" {
		t.Fatalf("checkpoint content = %q, want %q", got, "checkpoint")
	}
	if got := string(reviewContent); got != "review" {
		t.Fatalf("review content = %q, want %q", got, "review")
	}
}
