...
func (r *LargeScaleRefiner) fastPrune(p uint32, candidates []uint32, scoreBuf []scoredCandidate, maxScore int) []uint32 {
	...
	reducedDim := min(len(pVec), r.R*bits.Len(uint(r.R)))
	pVecR := pVec[:reducedDim]
	...
	for ; i+4 <= n; i += 4 {
		...
		v0, v1 := r.vectors[c0][:reducedDim], r.vectors[c1][:reducedDim]
		v2, v3 := r.vectors[c2][:reducedDim], r.vectors[c3][:reducedDim]
		m0, m1, m2, m3 := r.mags[c0], r.mags[c1], r.mags[c2], r.mags[c3]
		d0, d1 := vek32.Dot(pVecR, v0), vek32.Dot(pVecR, v1)
		d2, d3 := vek32.Dot(pVecR, v2), vek32.Dot(pVecR, v3)
		if m0 == 0 {
	...
	for ; i < n; i++ {
		c := toScore[i]
		cVec := r.vectors[c][:reducedDim]
		cMag := r.mags[c]
		var dist float64
		if cMag == 0 {
			dist = 2.0
		} else {
			dot := vek32.Dot(pVecR, cVec)
