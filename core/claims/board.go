...
func (b *ClaimsBoard) PostAction(ctx context.Context, action Action, inputClaims []Claim) error {
	...
	action = normalizeLegacyPostAction(action)
	normalizedClaims, err := normalizeLegacyPostClaims(action, inputClaims)
	if err != nil {
		return err
	}
	generated, err := b.GenerateClaimAction(ctx, action, normalizedClaims, GenerateClaimActionOptions{
		IdempotencyKey:                 action.IdempotencyKey,
		AllowMissingSubject:            legacyClaimsAllowMissingSubject(normalizedClaims),
		Reason:                         "claim generated by PostAction compatibility wrapper",
		SuppressGeneratedNotifications: true,
	})
	if err != nil {
		return err
	}
	claimIDs := make([]string, 0, len(generated.Claims))
	for i := range generated.Claims {
		claimIDs = append(claimIDs, generated.Claims[i].ID)
	}
	if err := b.PostGeneratedClaims(ctx, claimIDs, generated.Action.AgentID, ClaimPostOptions{
		AllowMissingSubject: legacyClaimsAllowMissingSubject(generated.Claims),
		Reason:              "claim posted by PostAction compatibility wrapper",
	}); err != nil {
		return err
	}
	b.hydratePostedClaims(inputClaims, claimIDs)
	return nil
}

func (b *ClaimsBoard) hydratePostedClaims(input []Claim, claimIDs []string) {
	for i, claimID := range claimIDs {
		if i >= len(input) {
			return
		}
		if claim, ok := b.CloneClaim(claimID); ok {
			input[i] = *claim
		}
	}
}
...
func normalizeLegacyPostAction(action Action) Action {
	if action.Type == "" {
		action.Type = ActionTypeTask
	}
	return action
}

func normalizeLegacyPostClaims(action Action, input []Claim) ([]Claim, error) {
	claims := make([]Claim, len(input))
	for i := range input {
		claims[i] = *CloneClaimEntity(&input[i])
		if strings.TrimSpace(claims[i].Description) == "" {
			claims[i].Description = strings.TrimSpace(claims[i].Title)
		}
		if action.Type == "" {
			claims[i].ActionType = ActionTypeTask
		}
		if IssuerAgentID(claims[i].Relations) == "" && strings.TrimSpace(action.AgentID) != "" {
			claims[i].Relations = append(claims[i].Relations, Relation{Related: strings.TrimSpace(action.AgentID), RelatedType: RelatedTypeAgent, Relationship: RelationshipIssuer})
		}
		if err := normalizeLegacyClaimValidations(&claims[i]); err != nil {
			return nil, err
		}
	}
	return claims, nil
...
func normalizeLegacyClaimValidations(claim *Claim) error {
...
func legacyClaimsAllowMissingSubject(claims []Claim) bool {
	for i := range claims {
		if SubjectAgentID(claims[i].Relations) == "" {
			return true
		}
	}
	return false
...
func (b *ClaimsBoard) SubmitTestaments(ctx context.Context, action Action, testaments []Testament) error {
	...
	action = normalizeLegacyTestamentAction(action)
	generated, err := b.GenerateTestamentAction(ctx, action, normalizeLegacyTestaments(action, testaments), GenerateTestamentActionOptions{
		IdempotencyKey:                 action.IdempotencyKey,
		AllowStandalone:                legacyTestamentsAllowStandalone(testaments),
		AllowEmptyArtifactReference:    true,
		Reason:                         "testament generated by SubmitTestaments compatibility wrapper",
		SuppressGeneratedNotifications: true,
	})
	if err != nil {
		return err
	...
	return b.PostGeneratedTestaments(ctx, testamentIDs, generated.Action.AgentID, TestamentPostOptions{
		Reason: "testament posted by SubmitTestaments compatibility wrapper",
	})
}

func normalizeLegacyTestamentAction(action Action) Action {
	action.Type = ActionTypeTestament
	return action
}

func normalizeLegacyTestaments(action Action, input []Testament) []Testament {
	testaments := make([]Testament, len(input))
	for i := range input {
		testaments[i] = *CloneTestamentEntity(&input[i])
		if strings.TrimSpace(testaments[i].AgentID) == "" {
			testaments[i].AgentID = strings.TrimSpace(action.AgentID)
		}
		if strings.TrimSpace(testaments[i].Summary) == "" && strings.TrimSpace(testaments[i].Context) == "" {
			testaments[i].Summary = "testament"
		}
	}
	return testaments
}

func legacyTestamentsAllowStandalone(testaments []Testament) bool {
	for i := range testaments {
		if ClaimIDFromRelations(testaments[i].Relations) == "" {
			return true
		}
	}
	return false
...
func (b *ClaimsBoard) ClaimsByLifecycleStatus(status ClaimLifecycleStatus) []*Claim {
	if !status.Valid() {
		return nil
	}
	b.mu.RLock()
	defer b.mu.RUnlock()
	filtered := make([]*Claim, 0)
	for _, claim := range b.claims {
		if claim.LifecycleStatus == status {
			filtered = append(filtered, CloneClaimEntity(claim))
		}
	}
	return filtered
}

func (b *ClaimsBoard) TestamentsByLifecycleStatus(status TestamentLifecycleStatus) []*Testament {
	if !status.Valid() {
		return nil
	}
	b.mu.RLock()
	defer b.mu.RUnlock()
	filtered := make([]*Testament, 0)
	for _, testament := range b.testaments {
		if testament.LifecycleStatus == status {
			filtered = append(filtered, CloneTestamentEntity(testament))
		}
	}
	return filtered
}

func (b *ClaimsBoard) ClaimsForAgentByLifecycleStatus(agentID, relationship string, status ClaimLifecycleStatus) []*Claim {
	if !status.Valid() {
		return nil
	}
	all := b.ClaimsForAgent(agentID, relationship)
