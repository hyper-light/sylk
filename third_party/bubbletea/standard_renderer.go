...
func (r *standardRenderer) flush() {
	...
	if frame != nil && len(frame.Lines) > 0 {
		newLines = append([]string(nil), frame.Lines...)
	}
	...
	if r.canIncrementallyFlush(frame, flushQueuedMessages, newLines) {
		r.flushIncrementalFrame(buf, frame, newLines)
		r.pendingFrame = nil
		return
	}
...
func (r *standardRenderer) writeFrame(s string, frame *FrameData) {
	r.mtx.Lock()
	defer r.mtx.Unlock()
	r.buf.Reset()
	r.pendingFrame = nil
	...
	if frame != nil {
		cloned := *frame
		cloned.Lines = append([]string(nil), frame.Lines...)
		cloned.DirtyRows = append([]int(nil), frame.DirtyRows...)
		r.pendingFrame = &cloned
	}
...
func (r *standardRenderer) canIncrementallyFlush(frame *FrameData, flushQueuedMessages bool, newLines []string) bool {
	if frame == nil || frame.Full || flushQueuedMessages || !r.altScreenActive {
		return false
	}
	if len(newLines) == 0 || len(r.lastRenderedLines) == 0 {
		return false
	}
	if len(newLines) != len(r.lastRenderedLines) {
		return false
	}
	return len(normalizeDirtyRows(frame.DirtyRows, len(newLines))) > 0
}

func (r *standardRenderer) flushIncrementalFrame(buf *bytes.Buffer, frame *FrameData, newLines []string) {
	dirtyRows := normalizeDirtyRows(frame.DirtyRows, len(newLines))
	if len(dirtyRows) == 0 {
		r.lastRender = r.buf.String()
		r.lastRenderedLines = newLines
		r.altLinesRendered = len(newLines)
	...
	for _, row := range dirtyRows {
		if _, ignore := r.ignoreLines[row]; ignore {
			continue
		}
		line := newLines[row]
		if r.width > 0 {
		...
		buf.WriteString(ansi.CursorPosition(0, row+1))
		_, _ = buf.WriteString(line)
	}
	buf.WriteString(ansi.CursorPosition(0, len(newLines)))
