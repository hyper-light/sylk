...
func (p *Program) eventLoop(model Model, cmds chan Cmd) (Model, error) {
	for {
		...
		case msg := <-p.msgs:
			...
			p.renderModel(model) // send view to renderer
		}
	}
}
...
func (p *Program) renderModel(model Model) {
	view := model.View()
	if frame, ok := extractModelFrameData(model); ok {
		p.renderer.writeFrame(view, &frame)
		return
	}
