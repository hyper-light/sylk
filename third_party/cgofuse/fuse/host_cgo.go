...
#define CGOFUSE_GET_API(n)		\
	...
	// Sylk vendor patch: original upstream cgofuse v1.6.0 only dlopens
	// /usr/local/lib paths. Homebrew on Apple Silicon installs to
	// /opt/homebrew/lib, so users with a brew-installed FUSE-T or macFUSE
	// fall through to failure. Extend the chain to cover both prefixes in
	// the same preference order: macFUSE (v4+) → osxfuse (legacy) → FUSE-T,
	// each under /usr/local first then /opt/homebrew as fallback.
	h = dlopen("/usr/local/lib/libfuse.2.dylib", RTLD_NOW); // MacFUSE/OSXFuse >= v4 (Intel / .pkg install)
	if (0 == h)
		h = dlopen("/opt/homebrew/lib/libfuse.2.dylib", RTLD_NOW); // MacFUSE/OSXFuse >= v4 (Apple Silicon Homebrew)
	if (0 == h)
		h = dlopen("/usr/local/lib/libosxfuse.2.dylib", RTLD_NOW); // MacFUSE/OSXFuse < v4 (Intel / .pkg)
	if (0 == h)
		h = dlopen("/opt/homebrew/lib/libosxfuse.2.dylib", RTLD_NOW); // MacFUSE/OSXFuse < v4 (Apple Silicon Homebrew)
	if (0 == h)
		h = dlopen("/usr/local/lib/libfuse-t.dylib", RTLD_NOW); // FUSE-T (Intel / .pkg install)
	if (0 == h)
		h = dlopen("/opt/homebrew/lib/libfuse-t.dylib", RTLD_NOW); // FUSE-T (Apple Silicon Homebrew)
