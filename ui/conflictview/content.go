...
func (m *Model) renderContent(vpH int) string {
	...
	for d := start; d < end; d++ {
		...
		switch region {
		case lineMarkerOurs:
			rendered = oursMarkerSt.Render(line)
		case lineMarkerDiv:
			rendered = divMarkerSt.Render(line)
		case lineMarkerTheirs:
			rendered = theirsMarkerSt.Render(line)
		default:
...
// renderActionLine renders the clickable "Accept Ours | Accept Theirs | Accept Both"
...
func (m *Model) renderActionLine(hunkIdx, width int) (string, actionTarget) {
	...
	oursLabel := oursSt.Render("Accept Ours")
	oursW := lipgloss.Width(oursLabel)
	...
	theirsLabel := theirsSt.Render("Accept Theirs")
	theirsW := lipgloss.Width(theirsLabel)
	...
	bothLabel := bothSt.Render("Accept Both")
	bothW := lipgloss.Width(bothLabel)
	target.bothRange = [2]int{col, col + bothW}
	b.WriteString(bothLabel)
...
func (m *Model) renderNonContentConflict(entry *ConflictFileEntry, vpH int) string {
	...
	case TypeDeleteModify:
		...
		set(padLine("  "+mutedSt.Render("Ours: file "+oursDesc), w))
		set(padLine("  "+mutedSt.Render("Theirs: file "+theirsDesc), w))
	case TypeBinary:
