...
// navBarHeight is the number of lines consumed by the navigation bar.
...
// navBarZones records click zones for the navigation bar.
type navBarZones struct {
	prevHunk [2]int // [start, end) visual column; zero-width means absent
	nextHunk [2]int
	ours     [2]int
	theirs   [2]int
	both     [2]int
...
func (m *Model) viewportHeight() int {
	overhead := toolbarHeight + navBarHeight + fileHeaderHeight
	if m.findActive {
...
func (m *Model) View(cursorVisible bool) string {
	...
	parts = append(parts, m.renderNavBar())
...
func (m *Model) handleLeftClick(msg tea.MouseMsg) tea.Cmd {
	...
	navRow := tbRow - navBarHeight
	if m.findActive {
		navRow -= findBarHeight
	}

	// Toolbar zone.
	if msg.Y >= tbRow {
		idx := conflictToolbarHitTest(m.width, msg.X)
		if idx >= 0 {
	...
	// Nav bar zone.
	if msg.Y >= navRow && msg.Y < navRow+navBarHeight {
		return m.handleNavBarClick(msg.X)
	}
...
// handleNavBarClick dispatches a click on the navigation bar.
func (m *Model) handleNavBarClick(x int) tea.Cmd {
	z := m.navZones
	if z.prevHunk[0] < z.prevHunk[1] && x >= z.prevHunk[0] && x < z.prevHunk[1] {
		m.prevHunk()
		return nil
	}
	if z.nextHunk[0] < z.nextHunk[1] && x >= z.nextHunk[0] && x < z.nextHunk[1] {
		m.nextHunk()
		return nil
	}
	if z.ours[0] < z.ours[1] && x >= z.ours[0] && x < z.ours[1] {
		return m.resolveKey(ResOurs)
	}
	if z.theirs[0] < z.theirs[1] && x >= z.theirs[0] && x < z.theirs[1] {
		return m.resolveKey(ResTheirs)
	}
	if z.both[0] < z.both[1] && x >= z.both[0] && x < z.both[1] {
