...
func TestPipelineWorkerAgentID_DoesNotSynthesizeTaskScopedIdentity(t *testing.T) {
	ctx := container.WithCreationContext(context.Background(), container.ContainerSpec{}, container.PodID("task-7"))

	if got := pipelineWorkerAgentID(ctx, "engineer"); got != "" {
		t.Fatalf("pipelineWorkerAgentID() = %q, want empty", got)
	}
}

func TestTaskScopedCreationAgentID_ReturnsEmptyForTaskScopedWorkersWithoutOverride(t *testing.T) {
	ctx := container.WithCreationContext(context.Background(), container.ContainerSpec{}, container.PodID("task-7"))

	if got := taskScopedCreationAgentID(ctx, "tester-pipeline", "global-tester"); got != "" {
		t.Fatalf("taskScopedCreationAgentID() = %q, want empty for runtime-generated task worker IDs", got)
	}
}

func TestTaskScopedCreationAgentID_PreservesSingletonIdentityOutsideTaskPods(t *testing.T) {
	if got := taskScopedCreationAgentID(context.Background(), "engineer", "engineer-global"); got != "engineer-global" {
		t.Fatalf("taskScopedCreationAgentID() = %q, want %q", got, "engineer-global")
	}
}
