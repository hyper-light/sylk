...
func TestGlobalInspectorSystemPromptForContract_IncludesStrictGlobalReviewProtocol(t *testing.T) {
	...
	for _, want := range []string{
		...
		"If the audit requires authoritative DAG, workflow, task, or pipeline progress/state, call `challenge_orchestrator`.",
		"When a tester, orchestrator, or architect response arrives, call `process_global_validation` before choosing the next action.",
		...
		"Treat `load_plan_context`, `consult_librarian_style`, `consult_academic_approach`, and `consult_archivalist_context` as high-value audit tools.",
		"# Workspace Layers",
