...
func TestAcademicApplyToolCalls_AuthorResearchPaperContinueResearchIsNonTerminal(t *testing.T) {
	...
	resp := &providers.Response{
		ToolCalls: []providers.ToolCall{
			...
			},
			{
				ID:        "consult_1",
				Name:      "consult",
				Arguments: `{"target":"librarian","query":"This tool call should be deferred to the next turn."}`,
			},
		},
	}
	surface := &scriptedAcademicSurface{
		outputs: map[string]string{
			"author_research_paper": `{"continue_research":true,"summary":"Continue research before final paper synthesis.","user_message":"Gather grounded external sources before retrying author_research_paper."}`,
			"consult":               `{"summary":"unexpected consult"}`,
		},
	}
	ctx := WithAcademicTurnState(context.Background(), newAcademicTurnState(
	...
	if outcome.terminal {
		t.Fatalf("expected non-terminal outcome, got %#v", outcome)
	}
	state := AcademicTurnStateFromContext(ctx)
	if state == nil || state.TerminalAction() != "" {
		got := ""
		if state != nil {
			got = string(state.TerminalAction())
		}
		t.Fatalf("terminal action = %q, want empty", got)
	}
	if got := strings.Join(surface.calls, ","); got != "author_research_paper" {
		t.Fatalf("executed tools = %q, want only author_research_paper", got)
	}
