...
func BuildPipelineSystemContext(task *PipelineTaskInput) string {
	...
	if protocol := decodeMap(task.Context, "pipeline_protocol"); len(protocol) > 0 {
		...
		b.WriteString("- The normal top-level flow is Inspector -> Tester for initial tests -> Engineer/Designer for implementation -> Inspector for audit, unless an active challenge explicitly says otherwise.\n")
		...
		b.WriteString("- End each turn with challenge_agent, handoff_next, validate_work, finalize_pipeline, or handoff_to_green (inspector only).\n")
		b.WriteString("- Use handoff_next for ordinary top-level phase progression. Use validate_work only when you are answering an active challenge. Use challenge_agent only for targeted uncertainty in returned work.\n")
		b.WriteString("- Process another agent's challenge response with process_validation before deciding the next handoff, challenge, or finalize step.\n")
		b.WriteString("- After process_validation, you may do any final direct audit you still need in that same turn, but you must not end the turn without a concrete next-step protocol tool call.\n")
		b.WriteString("- After Tester hands back the initial tests, Inspector audits those tests and either challenges Tester or hands implementation to Engineer/Designer.\n")
		...
		b.WriteString("- After Engineer or Designer hand work back, Inspector audits the implementation first. If a specific gap remains, Inspector should challenge the responsible agent directly instead of immediately starting another tester loop.\n")
		b.WriteString("- Use finalize_pipeline only after the current inspector audit is complete and the challenge evidence needed for that audit has already been processed.\n")
		b.WriteString("- If the finalize_pipeline audit passes and tester evidence confirms the required tests are implemented and passing, Inspector must immediately invoke handoff_to_green instead of starting another audit loop.\n")
		b.WriteString("- Once a pipeline is ready for OT, that completed pipeline takes priority: Inspector must invoke handoff_to_green before continuing with any other queued work, other pipelines, or other reviews.\n")
