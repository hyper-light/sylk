...
func consultPeerSkill(cfg CrossPipelineSkillConfig) *skills.Skill {
	return skills.NewSkill("consult_peer").
		Description("Ask a peer agent (cross-pipeline specialist or knowledge agent) for their evidence on a shared concern. Asynchronous — the addressee sees your consult in their next ambient_context envelope and responds at their own cadence. Returns a consult_id you can causal_trace later to find the response.").
		Domain("fabric").
		Keywords("consult", "fabric", "cross-pipeline", "peer", "knowledge-agent").
		Priority(86).
