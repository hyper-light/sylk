...
type Config struct {
	...
	MaxToolRuns     int                    `json:"max_tool_runs"` // Default: 8
	LLMTimeout      time.Duration          `json:"llm_timeout"`   // Default: 15s
	...
	DAGConfig DAGBridgeConfig `json:"dag_config"`

	// Bootstrap safety deadline — maximum wait for SignalReady before the
...
func DefaultConfig() Config {
	return Config{
		...
		DAGConfig:          DefaultDAGBridgeConfig(),
	}
}
...
// =============================================================================
// Health Check Result Types (deterministic monitoring output)
// =============================================================================

// HealthCheckResult is the output of a single deterministic monitoring cycle.
type HealthCheckResult struct {
	CheckedAt    time.Time          `json:"checked_at"`
	SessionID    string             `json:"session_id"`
	AgentResults []AgentCheckResult `json:"agent_results"`
	NewAlerts    []HealthAlert      `json:"new_alerts,omitempty"`
	Summary      HealthSummary      `json:"summary"`
}

// Cost estimates the Ristretto storage cost from struct sizes.
func (r *HealthCheckResult) Cost() int64 {
	// Base struct overhead + time + string header
	cost := int64(200)
	cost += int64(len(r.SessionID))

	// Per-agent result: fixed fields + string lengths
	for i := range r.AgentResults {
		cost += int64(120) // fixed fields (2 HealthLevel, 3 int, 1 float64)
		cost += int64(len(r.AgentResults[i].AgentID))
	}

	// Per-alert: fixed fields + string lengths
	for i := range r.NewAlerts {
		cost += int64(100)
		cost += int64(len(r.NewAlerts[i].ID))
		cost += int64(len(r.NewAlerts[i].AgentID))
		cost += int64(len(r.NewAlerts[i].Message))
	}

	// Summary: fixed fields + active alerts
	cost += int64(60)
	for i := range r.Summary.ActiveAlerts {
		cost += int64(100)
		cost += int64(len(r.Summary.ActiveAlerts[i].ID))
		cost += int64(len(r.Summary.ActiveAlerts[i].AgentID))
		cost += int64(len(r.Summary.ActiveAlerts[i].Message))
	}

	return cost
}

// HasCritical returns true if any agent is at critical level.
func (r *HealthCheckResult) HasCritical() bool {
	for i := range r.AgentResults {
		if r.AgentResults[i].Level == HealthLevelCritical {
			return true
		}
	}
	return false
...
// CriticalAgentIDs returns agent IDs at critical level.
func (r *HealthCheckResult) CriticalAgentIDs() []string {
	var ids []string
	for i := range r.AgentResults {
		if r.AgentResults[i].Level == HealthLevelCritical {
			ids = append(ids, r.AgentResults[i].AgentID)
		}
	}
	return ids
}

// AgentCheckResult is per-agent health state from one monitoring cycle.
type AgentCheckResult struct {
	AgentID          string      `json:"agent_id"`
	Level            HealthLevel `json:"level"`
	PreviousLevel    HealthLevel `json:"previous_level"`
	MissedHeartbeats int         `json:"missed_heartbeats"`
	ActiveTasks      int         `json:"active_tasks"`
	ErrorRate        float64     `json:"error_rate"`
