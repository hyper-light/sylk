...
- [x] **UI-02** Command approval modal not routed. `cmd/tui_fetch_approval.go`. **Accept:** `app.Update` dispatches approval messages to modal; modal decision flows to `commandapproval.Authorize`. *Closed: verified wired. `ui/app.go:2066` routes `msg.CommandApprovalRequestMsg` to `handleCommandApprovalRequest`; keyboard/mouse dispatch reaches `handleCommandApprovalKey` (`ui/app.go:2601`) and `handleCommandApprovalMouse` (`ui/app_editor_mouse.go:1905`); `commitCommandApproval` (`ui/app_editor_mouse.go:1281`) publishes a typed `guide.Message{Type: Response, Payload: {decision, approved, reason}}` on `guide.TopicResponses("guardian", target)`. The agent-side `commandapproval.Authorize` blocks on that same topic (see `core/commandapproval/context.go`). `ui/app_command_approval_test.go` covers queueing, modal lifecycle, and decision publish.*
- [x] **UI-03** LSPBridge diagnostics not forwarded. `ui/app_bootstrap.go:187-189`. **Accept:** LSPBridge.Start emits diagnostics via `TopicLSPDiagnostic`; app handler surfaces them in editor gutter. *Closed: verified wired. `ui/bridge/lsp.go:62` drains `manager.Diagnostics()` and emits `msg.LSPDiagnosticMsg{ServerID, FilePath, Diagnostics}`. `ui/app.go:2185` routes it to `handleLSPDiagnostic` (`ui/app_lsp.go:77`), which fans out to `editorOverlay.Update`, `focusedEditor().Update`, and `codePanel.SetDiagnostics`. Editor gutter rendering consumes via `ui/editor/model.go:2731-2774`. End-to-end path is live; no code change required this session.*
...
- [x] **UI-12** ActivityBridge/TokenUsageBridge not GoroutineScope-tracked. `ui/bridge/bridge.go`. **Accept:** both accept scope; sync.Once removed. *Closed: every UI bridge is now constructed with a `*concurrency.GoroutineScope` and registers its drain goroutine via `scope.Go(name, drainTimeout, fn)`. `ActivityBridge` (`ui/bridge/activity.go:50`), `TokenUsageBridge` (`ui/bridge/token_usage.go:33,63`), `StreamBridge` (`ui/bridge/stream.go:133,181`), `GuideBridge` (`ui/bridge/guide.go:42,96`), `LSPBridge` (`ui/bridge/lsp.go:31,44`), and `SessionBridge` (`ui/bridge/session.go:34,48`) all follow the same pattern. The naive `sync.Once` on close(done) was replaced by the scope-watch idiom (`scopeWatcher`) that is idempotent across repeated Stop() calls. `ui/bridge/activity_test.go:11` and `ui/bridge/token_usage_test.go:11` lock the idempotent-Stop contract.*
