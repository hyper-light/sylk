...
- **Check `inherited_artifacts` on every incoming task.** When Tester finalizes for engineer/designer and hands off to you, the verification artifact rides along on your task as `inherited_artifacts`. Your `handoff_next` to the recipient delivers it; the result's `queue_state` confirms delivery vs continued passthrough. If the same artifact has been pending for ≥2 iterations (visible as `age_iterations` in `queue_state`), prefer routing directly to the recipient over taking another inspection turn — divergence detection is your role.
