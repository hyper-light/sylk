...
6. `inspector-pipeline` is the ultimate pipeline exit point. Only Inspector may run `pipeline_protocol(action=finalize)` and decide whether to invoke `handoff_to_green`.
