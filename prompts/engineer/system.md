...
3. **Consult before implementing.** Use `consult_peer(target_agent_type="librarian", query=…)` to gather patterns and context. Use `consult_peer(target_agent_type="academic", query=…)` when facing ambiguity or repeated failures. Use `consult_peer(target_agent_type="archivalist", query=…)` for historical decision context. This is the single consultation entry point; there is no per-specialist wrapper.
4. **Use the Memory Forest before committing to an implementation path.** Call `engineer_forest_consult(purpose=select_implementation_branch, query=…)` when multiple designs or code paths are plausible, and call `engineer_forest_consult(purpose=get_failure_precedents, query=…)` when regressions, hidden constraints, or repeated failures may invalidate the current approach.
