...
Use the Memory Forest alongside `knowledge_query`: call `academic_forest_consult(purpose=get_authority_bundle, query=…)` to recall precedent and authority-weighted evidence before broadening the search, and call `academic_forest_consult(purpose=check_contradictions, query=…)` before presenting a claim as decisive when conflicting evidence may exist.
