...
- **Drive resolution.** When the audit returns blocking conflicts, use `request_correction` with the conflicting activity_id(s) in your evidence to drive the responsible pipeline back to reconciliation. Don't accept work whose scope contains unresolved cross-pipeline disputes.
