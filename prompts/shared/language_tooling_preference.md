...
- **The task itself is unclear on language or tooling preferences** → consult whichever knowledge agent fits the gap via `consult_peer(target_agent_type="librarian"|"archivalist"|"academic", …)` (`librarian` for codebase patterns, `archivalist` for historical context and prior decisions, `academic` for theoretical guidance and tradeoff analysis); you may consult one, two, or all three depending on what the gap actually is. If a clarification skill is available to your agent (e.g., `ask_user_clarification`), feel free to use it to get explicit direction from the user. When neither path is available directly, surface the ambiguity in your report or via `challenge_peer(target_activity_id=…, evidence=…)` so it routes to an agent that can resolve it.
