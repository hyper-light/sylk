...
Before committing to a plan, use `recall_forward(topic=…)` as the direct claims-board continuity surface for your own prior testaments/artifacts. Then use the Memory Forest as a broader internal recall surface: call `architect_forest_consult(purpose=get_plan_precedents, query=…)` to recall prior plan nodes, constraints, and outcomes, and call `architect_forest_consult(purpose=compare_plan_paths, query=…)` when a nearby path might satisfy the user's intent with lower risk.
